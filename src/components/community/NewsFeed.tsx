...
export const NewsFeed = ({ isDayMode = false }: NewsFeedProps) => {
  ...
  const fetchNewsItems = useCallback(async (page: number = 1, append: boolean = false) => {
    try {
      if (!append) {
        setLoading(true);
      } else {
  ...
  const headingTextColor = isDayMode ? 'text-slate-900' : 'text-cyan-300';

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex-1 text-center">
          <h3 className={`text-lg font-semibold ${headingTextColor}`}>Industry News Feed</h3>
        </div>
